...
```
2. **Host `assetlinks.json`**: Serve at `https://yourdomain.com/.well-known/assetlinks.json`.
```json
[{
...
```dart
// Replaces the current route stack (Declarative)
context.go('/details/123');

// Pushes onto the existing stack — back button works (Imperative)
context.push('/details/123');

// Named route with path parameters
